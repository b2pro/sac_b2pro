...
test("comentario e transicao notificam o atendente ao vivo, o dropdown abre o ticket e marcar todas zera o sino", async ({
  ...
  try {
    ...
    await pageA.getByRole("button", { name: "Enviar para análise" }).click()
    await expect(pageA.getByText("Aguardando análise").first()).toBeVisible()

    // --- B ve o sino subir para 2, SEM navegar nem recarregar a pagina ---
    await expect(pageB.getByText("2 notificações não lidas")).toBeVisible()
    ...
    await pageB.getByRole("button", { name: "Notificações" }).click()
    const itemTransicao = pageB.getByRole("menuitem", {
      name: new RegExp(`Ticket enviado para análise.*#${ticket.number}(\\D|$)`),
    })
    ...
    await expect(pageB.getByText("1 notificação não lida")).toBeVisible()
